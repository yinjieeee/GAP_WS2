...
\begin{frame}[plain]
  \begin{tikzpicture}[remember picture, overlay]
    ...
    \node[white,font=\large] at ([yshift=-1.2cm]current page.center){OAM $\cdot$ Laguerre--Gaussian Modes $\cdot$ TMDs $\cdot$ Literature Review $\cdot$ Motivation};
  \end{tikzpicture}
\end{frame}
...
\begin{frame}[plain]
  \begin{tikzpicture}[remember picture, overlay]
    ...
    \node[white,font=\large] at ([yshift=-1.2cm]current page.center){FoMs $\cdot$ Structures $\cdot$ Far-Field $\cdot$ Phase winding $\cdot$ Chirality Propagation $\cdot$ Valley Selectivity $\cdot$ Device Energy Efficiency $\cdot$ Comparison};
  \end{tikzpicture}
\end{frame}
