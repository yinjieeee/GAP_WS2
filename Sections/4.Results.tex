...
\centering
	...
	\includegraphics[width=0.7\textwidth]{img/rcp.png}
	\\(b) Cross polarization (RCP) 
 \caption{Comparison of far-field OAM mode purity under polarization cross-excitation ($\ell = 3$). (a) Target polarization (LCP) and (b) cross polarization (RCP) sources illuminating the same optimized structure. When the RCP source is used, the $\ell = 3$ mode purity drops sharply to only $0.03\%$, confirming that the structure has very strong circular polarization selectivity.} 
 \label{fig:ff_purity}
 \end{figure}
