...
Examining these six structures reveals a clear geometric evolution: the order of the spiral symmetry at the center of each structure exactly matches the target topological charge $\ell$. $\ell = 0$ shows full rotational symmetry with a closed GaP circular core at the center; $\ell = 1$ through $\ell = 5$ successively display one-arm, two-arm, up to five-fold spiral symmetry features. This geometric character shows that the optimizer, guided by gradients, spontaneously translates the required azimuthal phase modulation into a rotationally symmetric spiral design strategy. This closely parallels the working principle of a spiral phase plate (SPP), while the details are richer—the surrounding scattering features serve as compensation mechanisms for correcting near-field coupling errors, further improving far-field mode shaping accuracy.

As topological charge increases, the geometric complexity of the central spiral grows, and the surrounding scattering features become denser, reflecting the higher modulation demand on the phase space for higher-order OAM modes.
...
\centering
    ...
    \end{tabular}
\caption{Final converged GaP/air binary material density distributions for $\ell = 0$ to $5$ ($\beta = 512$, binarized structures). Black represents GaP ($n = 3.31$); white represents air ($n = 1$). At the center of each structure, $\ell$-fold spiral symmetry corresponding to the target topological charge is visible.}
